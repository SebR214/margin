...
pages monthly and APPENDS what it observed; this script is not the ongoing
mechanism, and it refuses to overwrite an existing file so a re-run cannot
truncate that history. Pass --force to reseed deliberately.
...
def main():
    ...
    if os.path.exists(WITHDRAWALS) and os.path.getsize(WITHDRAWALS) > 0 \
            and "--force" not in sys.argv:
        # The file is an append-only observation log once the watcher is running.
        # Silently rewriting it would erase the record of a floating fee, which
        # is the one thing that record exists to show.
        sys.exit(f"  {os.path.relpath(WITHDRAWALS, HERE)} already exists -- "
                 f"refusing to truncate an append-only log. Use --force to reseed.")
